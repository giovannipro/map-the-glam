...
def get_original_img_size(f_name):

	time()

	f_in = folder + "/" + f_name + ".tsv"  # test / data
	f_out = folder + "/" + f_name + "-original_img_size_2.tsv" 
			...
			for image in filecontent:
				...
				with open(f_out, "a") as f:
					try:
						...
						title = raw_data.find("tr").find("span",{"class":"fn"}).get_text()
						place = raw_data.findAll("tr")[1].findAll("td")[1].get_text()
						date = raw_data.findAll("tr")[2].findAll("td")[1].get_text()
						medium = raw_data.findAll("tr")[3].findAll("td")[1].get_text()
						dimension = raw_data.findAll("tr")[4].findAll("td")[1].get_text()
						photographer = raw_data.findAll("tr")[5].find("a",{"class":"extiw"}).get_text()
						print(medium)

						w = dimension.split(s)[2]
						h = dimension.split(s)[0]
						print(w + s + h)
						...
						# +  t + title.replace(n,"")
						output = str(index) + t + image.replace(n,"") + t + title.replace(n,"") + t + place.replace(n,"") + t + medium.replace(n,"") + t + dimension.replace(n,"") + t + w + t + h + t + photographer.replace(n,"")
						output_ = output + n

						f.write(output_)
						print(index)
					except:
						print(str(index) + t + image.replace(n,"") + t + "error")
						f.write(str(index) + t + image.replace(n,"") + t + "error" + n)
						pass
	time()
